...
export function ReportingView() {
  ...
  const [liveState, setLiveState] = useState<ChatState | null>(null);
  useEffect(() => {
  ...
  const metrics = [
    { 
      ...
      val: liveState?.reporting?.totalOperations?.toLocaleString() || '14,282', 
      icon: Activity, 
      color: 'text-cyan-500' 
    },
    { 
      label: 'Avg Latency', 
      val: `${liveState?.reporting?.avgLatency || 22}ms`, 
      icon: Zap, 
      color: 'text-yellow-500' 
    },
    { 
      label: 'Security Score', 
      val: liveState?.reporting?.securityScore?.toFixed(1) || '99.9', 
      icon: Shield, 
      color: 'text-emerald-500' 
    },
    { 
      label: 'Stored Snapshots', 
      val: liveState?.faultTolerance?.redundantSnapshotsCount?.toString() || '142', 
      icon: Database, 
      color: 'text-zinc-400' 
    },
  ];
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-10 lg:py-12 space-y-8 animate-fade-in">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        ...
        <Button onClick={handleExport} disabled={isGenerating} className="bg-emerald-600 hover:bg-emerald-500 text-white font-mono text-xs shadow-glow-lg transition-transform hover:scale-105 active:scale-95">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {metrics.map((m, i) => (
          <Card key={i} className="bg-zinc-900/50 border-white/5 hover:bg-zinc-900 transition-colors">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card className="bg-zinc-900/50 border-white/5 p-6">
          ...
          <div className="h-[250px] mt-4">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={defaultPerformanceData}>
                <defs>
        ...
        <Card className="bg-zinc-900/50 border-white/5 p-6">
          ...
          <div className="h-[250px] mt-4">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={defaultPerformanceData}>
                <CartesianGrid strokeDasharray="3 3" stroke="#27272a" vertical={false} />
                <XAxis dataKey="day" stroke="#52525b" fontSize={10} axisLine={false} tickLine={false} />
                ...
                />
                <Line type="monotone" dataKey="recoveryTime" stroke="#ef4444" strokeWidth={2} dot={{ r: 4, fill: '#ef4444', strokeWidth: 0 }} activeDot={{ r: 6, strokeWidth: 0 }} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </Card>
