...
import { Terminal, Send, Trash2, Cpu, ShieldCheck, Zap, Info } from 'lucide-react';
    ...
    <div className="flex flex-col h-[calc(100vh-280px)] min-h-[500px] bg-black border border-emerald-500/20 rounded-lg overflow-hidden shadow-[0_0_30px_rgba(16,185,129,0.05)] font-mono animate-fade-in">
      ...
      <div className="bg-zinc-900/80 px-4 py-2 border-b border-white/10 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
           {activeContext && (
             <div className="flex items-center gap-2 px-2 py-0.5 rounded bg-emerald-500/10 border border-emerald-500/20 text-[9px] text-emerald-500 font-bold">
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-6 space-y-4 scrollbar-thin scrollbar-thumb-zinc-800">
        <div className="text-[10px] text-zinc-600 border-b border-zinc-800/50 pb-2 mb-4">
          NEXUS_CORE_HYPERVISOR :: [RESILIENCE_ACTIVE] :: [WINDOW_COMPRESSION_ON]
        </div>
        {messages.map((m) => (
          <div key={m.id} className={cn(
            ...
            {m.isSystemLog && (
               <div className="w-full flex items-center gap-2 text-[10px] text-yellow-500/70 py-1 font-mono italic">
            ...
            {!m.isSystemLog && (
              <div className={cn(
                ...
                <div className="flex items-center justify-between gap-4 mb-1">
                  ...
                  {m.skillInsight && (
                    <Badge variant="outline" className="text-[8px] h-3 px-1 border-cyan-500/30 text-cyan-500">
        ...
        {isLoading && (
          <div className="flex items-center gap-2 text-emerald-500/50 text-xs animate-pulse">
            ...
            <span>Resilience Check & Context Routing...</span>
          </div>
        )}
      </div>
      {/* Input Area */}
      <div className="p-4 bg-zinc-900/50 border-t border-white/5">
        <div className="relative flex items-center gap-2 bg-black rounded border border-emerald-500/20 px-3">
          ...
          <Input
            ...
            placeholder="System command or natural query..."
            className="flex-1 bg-transparent border-none focus-visible:ring-0 text-zinc-300 text-xs h-10 placeholder:text-zinc-800"
          />
          <Button size="icon" variant="ghost" onClick={handleSend} disabled={isLoading} className="text-emerald-500 hover:bg-emerald-500/10">
