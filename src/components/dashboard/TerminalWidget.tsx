...
import { Terminal, Send, Trash2, Zap, ShieldCheck, Sparkles, Command, Info, ChevronDown, ChevronRight, Cloud, Bot } from 'lucide-react';
...
export function TerminalWidget() {
  ...
  const renderContent = (m: Message) => {
    ...
    const isMcp = m.level === 'MCP_TOOL';
    const isDaemon = m.level === 'DAEMON';
    return (
      <div className="flex gap-3 text-[11px] leading-relaxed group">
        ...
        <div className="flex-1 min-w-0">
          <span className={cn(
            "uppercase font-bold shrink-0 mr-2", 
            m.role === 'user' ? 'text-cyan-500' : 
            isMcp ? 'text-cyan-400' :
            isDaemon ? 'text-violet-500' : 'text-emerald-500'
          )}>
            {m.role === 'user' ? 'LOCAL_OP>' : isMcp ? 'MCP_TOOL>' : isDaemon ? 'DAEMON>' : 'AGENT>'}
          </span>
          {isMcp && <Cloud className="inline-block w-3 h-3 text-cyan-400 mr-2" />}
          {isDaemon && <Bot className="inline-block w-3 h-3 text-violet-500 mr-2" />}
          <span className={cn(
            m.role === 'user' ? 'text-zinc-300' : 'text-emerald-400',
            isMcp && 'text-cyan-400 font-mono',
            isDaemon && 'text-violet-400 font-bold'
          )}>
            {m.content}
          </span>
        </div>
      ...
      <div className="flex flex-col h-[calc(100vh-280px)] min-h-[500px] bg-[#020202] border border-emerald-500/20 rounded-lg overflow-hidden font-mono shadow-[0_0_40px_rgba(16,185,129,0.05)]">
        <div className="bg-zinc-900/80 px-4 py-2 border-b border-white/10 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest">Nexus_Infrastructure_Shell_v17</span>
            <Badge className="bg-violet-500/10 text-violet-500 border-none text-[8px] h-4">DAEMON_MODE_ACTIVE</Badge>
          </div>
        ...
        <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-2">
          ...
          {isLoading && <div className="text-emerald-500/50 text-[11px] animate-pulse flex items-center gap-2 mt-2">
            <Sparkles className="w-3 h-3" /> SYNTHESIZING_RESPONSE...
          </div>}
        </div>
        <div className="p-3 bg-zinc-900/30 border-t border-white/5 space-y-2">
          ...
          <div className="relative flex items-center gap-2 bg-black rounded border border-emerald-500/20 px-3">
            ...
            <Input
              ...
              placeholder="Enter intent or command (e.g., /mcp-status)..."
